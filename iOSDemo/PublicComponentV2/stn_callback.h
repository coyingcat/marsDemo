...
public:
    ...
    //底层获取task要发送的数据
    bool Req2Buf(uint32_t _taskid, void* const _user_context, const std::string& _user_id, AutoBuffer& _outbuffer, AutoBuffer& _extend, int& _error_code, const int _channel_select, const std::string& host);
    //底层回包返回给上层解析
    ...
    //上报网络连接状态
    void ReportConnectStatus(int _status, int longlink_status);
